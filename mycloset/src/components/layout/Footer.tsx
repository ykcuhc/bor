...
const FOOTER_LINKS = {
  ...
  Selling: [
    { label: 'Open Store',      href: '/sell' },
    { label: 'Seller Support',  href: '/help/selling' },
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 lg:py-16">
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-8">
          ...
          <div className="col-span-2 sm:col-span-3 lg:col-span-1">
            <span className="text-xl font-bold text-white">Miova<span className="text-accent-400">.</span></span>
            <p className="mt-3 text-sm text-gray-400 leading-relaxed max-w-xs">
              Kuwait&apos;s marketplace for fashion, electronics, beauty, and more — from verified stores and community sellers.
            </p>
            <div className="flex gap-2 mt-5">
              {SOCIAL_LINKS.map(s => (
                <a key={s.label} href={s.href} aria-label={s.label}
                  className="w-8 h-8 rounded-full bg-gray-700 text-gray-300 flex items-center justify-center hover:bg-brand-600 hover:text-white transition-colors">
                  {s.icon}
                </a>
              ))}
            </div>
          ...
          {Object.entries(FOOTER_LINKS).map(([heading, links]) => (
            <div key={heading}>
              <h4 className="text-xs font-bold text-white uppercase tracking-wider mb-4">{heading}</h4>
